...
public interface VisitOfTeamRepository extends JpaRepository<VisitOfTeam, UUID> {
    ...
    @Query("select sum(sbv.selling_price*sbv.quantity) from SnacksAndBoissonsOfVisit sbv , VisitOfTeam vt where sbv.visitOfTeam=vt and vt.day >= :dateDebut and vt.day <= :dateFin")
