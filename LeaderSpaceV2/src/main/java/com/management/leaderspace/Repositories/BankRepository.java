...
public interface BankRepository extends JpaRepository<Bank, UUID> {
    ...
    List<Bank> filterFromBankByDate(@Param("dateDebut") LocalDate dateDebut, @Param("dateFin") LocalDate dateFin);
    @Query("SELECT d FROM Bank d WHERE d.somme >= 0 AND d.date >= :dateDebut AND d.date <= :dateFin ORDER BY d.date DESC")
    List<Bank> filterToBankByDate(@Param("dateDebut") LocalDate dateDebut, @Param("dateFin") LocalDate dateFin);
}
